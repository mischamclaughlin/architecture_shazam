...
def generate_description(audio_tags, model):
  ...
  Music features:
    ...
    • Key: {audio_tags['key']}  
    • (Optional) Spectral centroid: {audio_tags.get('mean_centroid', 'N/A')}  
    • (Optional) Mean RMS: {audio_tags.get('mean_rms', 'N/A')}  
    • (Optional) MFCC means: {', '.join(str(audio_tags.get(f'mfcc{i}_mean', '-')) for i in range(1, 14))}
  ...
  Instructions:
      ...
      3. Then, below that, write a 2-3 sentence “Overall look” summary (imagine a headline in an architecture magazine).
