...
for i, latent in enumerate(latents):
    ...
    # 1) grab geometry, handling both torch.Tensor and np.ndarray
    verts = raw_mesh.verts
    faces = raw_mesh.faces

    verts = (
        raw_mesh.verts
        if isinstance(raw_mesh.verts, np.ndarray)
        else raw_mesh.verts.cpu().numpy()
    )
    faces = (
        raw_mesh.faces
        if isinstance(raw_mesh.faces, np.ndarray)
        else raw_mesh.faces.cpu().numpy()
    )

    num_passes = 4
    for _ in range(num_passes):
        subdiv_result = subdivide(verts, faces)
        verts, faces = subdiv_result[0], subdiv_result[1]

    # 3) rebuild a Trimesh for export
    mesh_sub = trimesh.Trimesh(vertices=verts, faces=faces, process=True)

    filename = f"house_mesh_{i}_{timestamp}.obj"
    path = os.path.join(out_dir, filename)
    try:
        mesh_sub.export(path)
